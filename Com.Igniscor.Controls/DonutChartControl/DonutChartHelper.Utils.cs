...
internal static partial class DonutChartHelper
{
    ...
    private static SKPath CreateSectorSeparatorPath(float start, float end, float outerRadius, float innerRadius)
    {
        ...
        var angle = (DonutChartHelper.TotalAngle * start - DonutChartHelper.UprightAngle);
    ...
    private static SKPath CreateSectorPath(float start, float end, float outerRadius, float innerRadius)
    {
        ...
        var angle1 = DonutChartHelper.TotalAngle * start - DonutChartHelper.UprightAngle;
        var angle2 = DonutChartHelper.TotalAngle * end - DonutChartHelper.UprightAngle;
    ...
    private static SKPoint GetCirclePoint(float radius, float angle) =>
        new(radius * (float)Math.Cos(angle), radius * (float)Math.Sin(angle));
    ...
    private static float GetEmptySectorHeight(float squareSide, float textHeight) =>
        (squareSide - textHeight) / 2;
    ...
    private static SKBitmap GetSKBitmap(byte[] imageData)
    {
